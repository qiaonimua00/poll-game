...
extern unsigned char Mpu6050;
extern QueueHandle_t g_xQueueMPU6050;
...
void platform_task(void *params)
{
    ...
    while (1)
    {
			...
			if(Mpu6050)
			{
				xQueueReceive(g_xQueueMPU6050,&angx,0);
				if(angx>105)
				{
					uptMove = 1;
					vTaskDelay(8);
...
{		
	...
	/* 创建队列:平台任务从里面读到红外数据,... */
	 g_xQueueMPU6050 = xQueueCreate(10,sizeof(struct mpu6050_data));
